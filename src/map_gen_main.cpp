...
{
	int enemies = 1, w = 80, h = 100, terrain = 0, tech = 0;
	...
	for(int i = 1; i < argc; i++)
	{
		...
		else { printf("usage: %s -e <enemies 1-3> [-w tiles] [-h tiles] [-t terrain 0-4] [-L tech 0-5] [-s seed] [-o out.map]\n", argv[0]); return 1; }
	}

	return MapGen::Generate(out_path, enemies, w, h, terrain, tech, seed) ? 0 : 1;
}
