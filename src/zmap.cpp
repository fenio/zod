...
bool ZMap::CheckLoad()
{
	...
	if(tile_list.size() != (size_t)basic_info.width * basic_info.height)
	{
...
int ZMap::Write(const char* filename)
{
	...
	if((size_t)basic_info.width * basic_info.height != tile_list.size())
...
int ZMap::GetBlitInfo(SDL_Surface *src, int x, int y, SDL_Rect &from_rect, SDL_Rect &to_rect)
{
	...
	//This used to have its own (broken) clipping math that produced a
	//negative from_rect.w for on-screen sprites, so BlitSurface bailed out
	//(srcrect->w <= 0) and the sprite — e.g. zone flags — never drew.
	//Just delegate to the correct width/height overload.
	return GetBlitInfo(x, y, src->w, src->h, from_rect, to_rect);
}
