...
void ZSDL_Surface::LoadNewSurface(int w, int h)
{
	...
	new_surface = ZSDL_CreateSurface(w, h, 32, 0xFF000000, 0x0000FF00, 0x00FF0000, 0x000000FF);
...
void ZSDL_Surface::LoadBaseImage(SDL_Surface *sdl_surface_, bool delete_surface)
{
	...
	SDL_Surface *new_ret;
	new_ret = SDL_ConvertSurface(sdl_surface, SDL_PIXELFORMAT_ARGB8888);
...
void ZSDL_Surface::UseDisplayFormat()
{
	...
	SDL_Surface *new_ret;
	new_ret = SDL_ConvertSurface(sdl_surface, SDL_PIXELFORMAT_XRGB8888);
...
void ZSDL_Surface::MakeAlphable()
{
	...
	UseDisplayFormat();
	SDL_SetSurfaceColorKey(sdl_surface, true, 0x000000); 
}
...
void ZSDL_Surface::SetAlpha(unsigned char alpha_)
{
	...
	if(!use_opengl)
	{
		if(sdl_surface) ZSDL_SetSurfaceAlpha(sdl_surface, alpha);
		if(sdl_rotozoom) ZSDL_SetSurfaceAlpha(sdl_rotozoom, alpha);
	}
}
...
int ZSDL_Surface::GetMapBlitInfo(SDL_Surface *src, int x, int y, SDL_Rect &from_rect, SDL_Rect &to_rect)
{
	...
	//horizontal clip — the old else-branch produced a negative from_rect.w
	//for fully on-screen sprites, so the blit drew nothing (invisible
	//resume banner, etc). Mirror the correct width/height clipping.
	if(to_rect.x < 0)
	{
		from_rect.x = -to_rect.x;
		from_rect.w = src->w + to_rect.x;
		to_rect.x += from_rect.x;
		if(from_rect.w > view_w) from_rect.w = view_w;
	}
	else if(to_rect.x + src->w > view_w)
	{
		from_rect.x = 0;
