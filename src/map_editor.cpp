	...
	while(1)
	{
		...
		switch( event.type ) 
		{
			...
			case SDL_EVENT_WINDOW_RESIZED:
				...
				{
					SDL_SetVideoMode(event.window.data1, event.window.data2, 32, SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_RESIZABLE);
					edit_map.SetViewingDimensions(event.window.data1-MAP_SHIFT_X, event.window.data2);
					ZSDL_Surface::SetScreenDimensions(event.window.data1, event.window.data2);
					draw_everything();
				}
				break;
