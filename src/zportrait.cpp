...
int ZPortrait::GetBlitInfo(SDL_Surface *src, int x, int y, SDL_Rect &from_rect, SDL_Rect &to_rect)
{
	...
	//horizontal clip — old else-branch produced a negative from_rect.w for
	//fully on-screen sprites (same SDL2-port bug as the flags/resume banner),
	//so the animated portrait "hand" overlay drew nothing. Correct clipping:
	if(to_rect.x < 0)
	{
